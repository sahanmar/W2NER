...
from transformers import AutoModel, models
...
class Biaffine(nn.Module):
    def __init__(
        self, n_in: int, n_out: int = 1, bias_x: bool = True, bias_y: bool = True
    ) -> None:
...
class Model(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.bert: models.bert.modeling_bert.BertModel = AutoModel.from_pretrained(
        ...
        conv_input_size = (
            config.lstm_hid_size + config.dist_emb_size + config.type_emb_size
        )
        ...
        self.cln = LayerNorm(
            config.lstm_hid_size, config.lstm_hid_size, conditional=True
        )
    ...
    def forward(
        ...
        """
        bert_embs = self.bert(
            input_ids=bert_inputs, attention_mask=bert_inputs.ne(0).float()
        )
